...
export default function CreateChannelForm(): JSX.Element {
  ...
  function userChanged(user: UserObject, checked: boolean) {
    setFormData((currentFormData) => ({
      ...currentFormData,
      users: checked
        ? [...currentFormData.users, user]
        : currentFormData.users.filter((thisUser) => thisUser.id !== user.id),
    }));
  }

  function buttonDisabled(): boolean {
    return !formData.channelName || !formData.category || formData.users.length <= 1;
  }
